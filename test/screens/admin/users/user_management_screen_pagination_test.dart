...
import 'package:lms_mobile_flutter/screens/admin/users/user_management_screen.dart';
import 'package:lms_mobile_flutter/features/admin/users/admin_user_list_provider.dart' as users;
...
void main() {
  ...
  testWidgets('pagination buttons disabled/enabled and filter clear resets', (tester) async {
    // Provide a page-aware fake list so the pagination buttons update correctly
    users.AdminUserList makeList(int page) => users.AdminUserList(
          const [],
          users.Pagination(
            page: page,
            limit: 10,
    ...
    await tester.pumpWidget(
      wrapWithScaffoldBody(
        ...
        overrides: [
          users.adminUserListProvider.overrideWithProvider((filter) {
    ...
    final prevFinder = find.byKey(const ValueKey('users_pagination_prev'));
    for (var i = 0; i < 40 && prevFinder.evaluate().isEmpty; i++) {
      await tester.pump(const Duration(milliseconds: 25));
    }

  // Initial pagination: prev disabled, next enabled
  final nextFinder = find.byKey(const ValueKey('users_pagination_next'));
  ...
  final nextBtn1 = tester.widget<TextButton>(nextFinder);
    ...
    // Go to page 2
  await tester.tap(find.byKey(const ValueKey('users_pagination_next')));
